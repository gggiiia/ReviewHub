...
export function WidgetMasonry({ reviews }: WidgetMasonryProps) {
  return (
    <div className="[column-width:16rem] gap-4 sm:[column-width:18rem] md:[column-width:20rem] [column-fill:balance]">
      {reviews.length === 0 && (
        <div className="border rounded-md p-3 text-sm text-muted-foreground">No positive reviews yet.</div>
      )}
      {reviews.map((r) => {
