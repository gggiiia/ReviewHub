...
function SourceBadge({source}: { source: "google" | "facebook" }) {
    ...
    return (
        <span
            className="inline-flex items-center gap-1 rounded-xs px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-muted-foreground">
      {isGoogle ? (
    ...
    return (
        <div
            ...
            {reviews.map((r) => {
                ...
                return (
                    <div key={r.id} className="mb-4 break-inside-avoid border rounded-md p-3 bg-white">
                        <div className="flex items-start gap-3">
                            <div
                                className="size-10 shrink-0 rounded bg-muted flex items-center justify-center font-medium">
                                {initial}
                            </div>
                            <div className="min-w-0 w-full">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span
                                        className="text-muted-foreground text-xs">{new Date(r.date).toLocaleDateString()}</span>
